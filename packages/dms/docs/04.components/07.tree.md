...
`Tree` adds no permission options of its own. Like every component it is gated by the standard component permission — the page's permission id (its full id, unless the page declares `permission.id`) followed by every field/child id in the tree's position, e.g. `.files` for a top-level field. The routes behind `fetchUrl` / `lazyLoadUrl` are ordinary controllers with their own auth. See [Auth & permissions](/docs/auth-and-tenancy/auth-and-permissions).

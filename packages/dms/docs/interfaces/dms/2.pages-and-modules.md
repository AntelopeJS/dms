...
The third argument is the page layout, a `ComponentInfo` produced by a layout builder such as `DefaultLayout()` from the `dms-base` interface. When you omit it, the page uses `DefaultLayout()`.
...
| `authOnly: true` | `AuthUser` from the `dms-auth` interface: a logged-in user, no permission check |
| page under `/modules/` | `AuthOwnerOnly` from the `dms-auth` interface: module pages are platform-owner-only |
...
```typescript
import { StampUploadTokens } from "@antelopejs/interface-dms/page";

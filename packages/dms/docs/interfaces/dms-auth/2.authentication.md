...
This is how the DMS builds its tenant guards: `AuthTenantOwner` and `AuthTenantMember` in the `dms` interface both start from `authenticateRequestUser`, then resolve the request's tenant and check the caller's membership on top of it.

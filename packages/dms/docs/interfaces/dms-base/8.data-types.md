...
The `data-types` subpath re-exports the built-in catalogues, so `DefaultDataTypes`, `DefaultDataCompareTypes` and `StatusType` are all reachable from it. They also have their own modules, `data-types/default-types` and `data-types/status-type`, which is how much of the DMS imports them.

Note that the data types are **not** re-exported from the `dms-base` package root — always import them from the `data-types` subpath.

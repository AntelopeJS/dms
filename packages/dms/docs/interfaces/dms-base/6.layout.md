...
`Grid`, `GridRow`, `HStack`, `VStack`, `Spacer`, `Tab`, `Placeholder`, `DefaultLayout`, `FormPageLayout` and `EmptyLayout` are also exported from the package root. `CustomComponent` is only exported from `dms-base/custom`.
...
`CustomComponent(componentName)` returns a bare `ComponentBuilder` for a frontend component resolved by name. Nothing else in `dms-base` is involved: the options you pass with `.options()` are handed to that component as-is, and the component itself comes from a frontend module registered with `AddFrontendModule`.
...
Since the return value is an ordinary builder, `.options()`, `.child()`, `.meta()`, `.watch()`, `.onCreated()` and `.onFilter()` all apply, and the component can hold `dms-base` components as children.

...
`sendAdminInviteEmail` takes a bare address and a token instead of a `User`, because an invitee has no user row until they sign up. The invite record itself, its token and its expiry belong to the [Core](../1.core/1.introduction.md) domain, whose invite helper dispatches this mail for you.

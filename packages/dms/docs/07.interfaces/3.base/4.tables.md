...
| `isVisible` | `boolean` | Whether the column is shown by default. `false` hides it until the user picks it in the column menu. |
...
| `emptyState` | `TableViewEmptyState` | `title` and `description` (display strings, `$`-prefixed for i18n keys) shown instead of the generic empty-table message; a filtered empty table keeps its "no results" message. |

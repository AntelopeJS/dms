...
Registering from `onCreated` is what makes the binding follow the component: whichever page the component is placed on gets the topic. The chart builders of the [Base](../3.base/1.introduction.md) domain do exactly this — a chart declaring `realtimeTopic` registers each of its topics on the page it is placed on.

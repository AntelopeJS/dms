...
Components define their shortcuts with Nuxt UI's `defineShortcuts` as usual; the DMS part is **declaring** them so they appear on the built-in **Shortcuts** settings page. Each layer lists registry files in its `dms.frontend.ts`, and each registry default-exports the metadata:

```ts [frontend-vue/dms.frontend.ts]
export default defineFrontendConfig({
  dmsShortcuts: { registries: ["app/config/shortcuts-registry.ts"] },
});
```

...
The target is the **page class**, imported from its owning package — never a hardcoded page id, so a renamed page breaks at compile time. A page meant to be extended must therefore be exported from its package's public surface; the DMS's own pages live behind `@antelopejs/dms/pages`.
